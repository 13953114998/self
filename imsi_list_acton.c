...
void *thread_pare_imsi_action_list(void *arg)
{
	...
	do {
		...
		list_for_each_entry(tpos, imsi_head_allow_read, node) {
			...
			if (send_str) {
				send_string_udp(send_str, strlen(send_str));
				/* send to udp recv.... */
				free(send_str);
			}
#ifdef SAVESQL
			...
			strcpy(s_point.type, "baicell_4G");
			/* save to sqlite */
			memset(time_tmp, 0, len);
			timeinfo = localtime(&(tpos->rtime));
			snprintf(s_point.this_time, 20,
				"%04d-%02d-%02d %02d:%02d:%02d",
				timeinfo->tm_year + 1900, timeinfo->tm_mon + 1,
				timeinfo->tm_mday, timeinfo->tm_hour,
				timeinfo->tm_min, timeinfo->tm_sec);
			strcpy(s_point.dev_ip, tpos->ip);
			strcpy(s_point.imsi, (char *)((tpos->ptr).IMSI));
			strcpy(s_point.imei, (char *)((tpos->ptr).IMEI));
			save2sqlite_insert(s_point);/* insert to db */
#endif /* SAVESQL */
		}
