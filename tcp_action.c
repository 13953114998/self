...
void slation_data(int sockfd, unsigned char *buf, int buf_len)
{
	...
	script+=6;
	char id[7] = {0};
	sprintf(id, "%02x%02x%02x%02x%02x%02x", dev_id[0], dev_id[1], dev_id[2], dev_id[3], dev_id[4], dev_id[5]);
	printf("dev_id:%s\n", id);
		...
		switch(num)
		{
			case G_GPS_OBD_MESSAGE:/*GPS+OBD混合信息上传*/
				...
				}else
				{
					...
					memcpy(data, &buf[script], 3);
					script+=3;
					unsigned char time[3] = {0};
					sprintf(time, "%02u%02x%02x", xtoint(buf[script])+8, buf[script+1], buf[script+2]);
					script+=3;
