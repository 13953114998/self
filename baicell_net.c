...
/* 频点默认配置信息 */
def_band_config_t default_config[3] =
{
	/*上频,    下频,  PLMN,   带宽, 频段, 物区ID(pci),   区码,  小区ID, UePMax,EnodeBPMax */
//  { 19650,   1650, "46001",   25,    3,   501,        12345,   123,     23,     20},
    {0x4CC2,  0x672, "46001", 0x19,    3,   0x1F5,      0x3039,  0x7B,   0x17,   0x14}, /* FDD 46001 */
//  { 19825,   1825, "46000",   25,    3,   501,        12345,   123,     23,     20},
    {0x4D71,  0x721, "46011", 0x19,    3,   0x1F5,      0x3039,  0x7B,   0x17,   0x14}, /* FDD 46011 */
//  {   255,  37900, "46000",   25,   38,   501,        1264,    123,     23,     20},
    {  0xFF, 0x940C, "46000", 0x19, 0x26,   0x1F5,      0x4F0,   0x7B,   0x17,   0x14}, /* TDD 46000 */
};
band_entry_t *entry = NULL;
...
void bc_sta_json_init()
{
	...
	cJSON_AddStringToObject(bc_sta_object,"type", "08");
...
S32 init_baicell_socket()
{
	...
	S32 ret = 0;
	/* 设置地址复用(避免bing出错(Address already in used)) */
	int flag = 1;
	ret = setsockopt(bc_server_fd, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag));
	if(ret < 0) {
...
//	printf("send pakage (len:%u)to band:%s\n",send_len, ipaddr);
	...
	if (args_info.bc_proto_arg == BAICELL_PROTOCOL_TCP) {
		...
		if (ipaddr)
			...
			send_addr.sin_addr.s_addr = inet_addr(entry->ipaddr);
...
void send_reboot_request_to_baicell(U32 act)
{
	wrFLLmtToEnbRebootcfg_t pStr;
	memset(&pStr, '\0', sizeof(pStr));
	pStr.SelfActiveCfg = act;
	wrMsgHeader_t WrmsgHeader;
	WrmsgHeader.u16MsgLength = sizeof(pStr);
	WrmsgHeader.u16MsgType = O_FL_LMT_TO_ENB_REBOOT_CFG;
	WrmsgHeader.u32FrameHeader = MSG_FRAMEHEAD_DEF;
	WrmsgHeader.u16frame =
		(entry->work_mode == TDD)?SYS_WORK_MODE_TDD:SYS_WORK_MODE_FDD;
	WrmsgHeader.u16SubSysCode = SYS_CODE_DEFAULT;
#ifdef BIG_END
	pStr.SelfActiveCfg = my_htonl_ntohl(pStr.SelfActiveCfg);
	WrmsgHeader.u16MsgType = my_htons_ntohs( WrmsgHeader.u16MsgType);
	WrmsgHeader.u32FrameHeader = my_htonl_ntohl(WrmsgHeader.u32FrameHeader);
	WrmsgHeader.u16frame = my_htons_ntohs(WrmsgHeader.u16frame);
	WrmsgHeader.u16SubSysCode = my_htons_ntohs(WrmsgHeader.u16SubSysCode);
	...
	printf("send reboot config request command!\n");
...
void send_de_or_active_cell_req_to_baicell(U32 active)
{
	/* LTE */
	wrFLLmtToEnbSetAdminStateCfg_t pStr;
	memset(&pStr, '\0', sizeof(pStr));
	pStr.workAdminState = active;

	wrMsgHeader_t WrmsgHeader;
	WrmsgHeader.u16MsgLength = sizeof(pStr);
	WrmsgHeader.u16MsgType = O_FL_LMT_TO_ENB_SET_ADMIN_STATE_CFG;
	WrmsgHeader.u32FrameHeader = MSG_FRAMEHEAD_DEF;
	WrmsgHeader.u16frame =
		(entry->work_mode == TDD)?SYS_WORK_MODE_TDD:SYS_WORK_MODE_FDD;
	WrmsgHeader.u16SubSysCode = SYS_CODE_DEFAULT;
#ifdef BIG_END
	pStr.workAdminState = my_htonl_ntohl(pStr.workAdminState);
	WrmsgHeader.u16MsgType = my_htons_ntohs( WrmsgHeader.u16MsgType);
	WrmsgHeader.u32FrameHeader = my_htonl_ntohl(WrmsgHeader.u32FrameHeader);
	WrmsgHeader.u16frame = my_htons_ntohs(WrmsgHeader.u16frame);
	WrmsgHeader.u16SubSysCode = my_htons_ntohs(WrmsgHeader.u16SubSysCode);
	...
	case O_FL_ENB_TO_LMT_ARFCN_IND: //小区频点信息查询回复
		{
			...
			free(send_str);
			char contrl_cmd[32] = {0};
			snprintf(contrl_cmd, 32,
				"%s %u", CONTROL_PA_CMD,pStr->sysBand);
			system(contrl_cmd);
			break;
		}
	case O_FL_ENB_TO_LMT_TDD_SUBFRAME_ASSIGNMENT_QUERY_ACK:/* TDD子帧查询应答 */
		{
			...
			if (!object) break;
			cJSON_AddStringToObject(object, "ip", client_ip);
			cJSON_AddStringToObject(object, "tddsf_assignment",
				tddsf);
			cJSON_AddStringToObject(object, "tdd_special_patterns",
				tdd_);
			S8 *send_str = cJSON_PrintUnformatted(object);
			cJSON_Delete(object);
			if (!send_str) break;
...
#if 0
			/* 若在Idel态，下发激活小区 */
			if (pStr->u32CellState == IDLE_STATE) {
				wrFLLmtToEnbSetAdminStateCfg_t sStr;
				...
				WrmsgHeader.u16SubSysCode = my_htons_ntohs(WrmsgHeader.u16SubSysCode);
				WrmsgHeader.u16MsgType = my_htons_ntohs(WrmsgHeader.u16MsgType);
				sStr.workAdminState = my_htonl_ntohl(sStr.workAdminState);
#endif
...
//				pStr->u8ControlSubMode,
			...
			entry->ue_mode = pStr->u8WorkMode;
			memcpy(entry->ue_IMSI, pStr->IMSI, IMSI_LENGTH);
		...
		{/* Heartbeat */
			...
			if (entry->_auto == 3) {
				...
				safe_pthread_create(thread_send_generic_request_to_bc,
					(void *)(client_ip));
			} else if (entry->_auto >= 2 && !(entry->_auto % 20)) {
				...
				send_generic_request_to_baseband(client_ip,
					O_FL_LMT_TO_ENB_CELL_STATE_INFO_QUERY);
				send_generic_request_to_baseband(client_ip,
					O_FL_LMT_TO_ENB_GET_ARFCN);
			}
			break;
		}
	case O_FL_ENB_TO_LMT_SYS_MODE_ACK:
		{
			...
			if(pStr->CfgResult == 0) {
				...
				/* 工作模式配置成功 重启基带板 */
				send_reboot_request_to_baicell(0);
			} else {
	...
	case O_FL_ENB_TO_LMT_SYS_ARFCN_ACK: /* 频点配置应答 */
		{
			...
			S8 state_req[128] = {0};
			if(pStr->CfgResult == 0) {
				printf( "set baseBand(IP:%s) Arfcn Success!",
					client_ip);
				write_action_log(client_ip,
					"arfcn config success!");
				snprintf(state_req, 128,
					LEDINF_ARFCN_STATE_STR, 1);
			} else {
				printf("set baseBand(IP:%s) Arfcn Failed!",
					client_ip);
				snprintf(state_req, 128,
					LEDINF_ARFCN_STATE_STR, 0);
			}
			write_to_blue_tooth(get_bluetooth_fd(), state_req);
			if (entry->cellstate == IDLE_STATE)
				send_de_or_active_cell_req_to_baicell(1);
			break;
		}
	case O_FL_ENB_TO_LMT_SELF_ACTIVE_CFG_PWR_ON_ACK://上电自动建小区配置应答
		{
	...
	case O_FL_ENB_TO_LMT_MEAS_UE_ACK://4.2. 测量 UE 配置应答
		{
			...
			/* 查询UE测量配置信息 */
			send_generic_request_to_baseband(client_ip,
				O_FL_LMT_TO_ENB_MEAS_UE_CFG_QUERY);
			break;
		}
	...
	case O_FL_ENB_TO_LMT_SYS_ARFCN_MOD_ACK: //3.94小区频点动态修改应答
		{
			...
			if (!(pStr->CfgResult)) {
				...
				if (track_cfg->config_status < CFG_STATUS_SET_ARFCN_OK) {
					track_cfg->config_status =
...
{
	if (!arg) return NULL;
	band_entry_t *the_entry = (band_entry_t *)arg;
	U8 rec_buf[BAICELL_MAX_MSG_LENGTH] = {0};
	U32 ret = 0;
	U32 msg_len = 0;
	U32 header_len = sizeof(wrMsgHeader_t);
	int fd = the_entry->sockfd;
	while (1) {
		//ret = recv(fd, rec_buf, header_len, MSG_WAITALL);
		ret = recv(fd, rec_buf, header_len, 0);
		if (ret <= 0) {
		...
		if (msg_len > header_len) {
			ret += recv(fd, &rec_buf[ret],
				msg_len - header_len, 0);
		}
		baicell_pare_recv_value(rec_buf, the_entry);
...
void *thread_socket_to_baicell(void *arg)
{
	...
	if (args_info.bc_proto_arg == BAICELL_PROTOCOL_TCP) {
		...
		while(1) {
			...
			if (entry == NULL) {
				entry = (band_entry_t *)malloc(sizeof(band_entry_t));
				if (!entry) {
					perror("malloc entry mem failed!\n");
					exit(0);
				}
				...
				entry->ipaddr =
					strdup(inet_ntoa(bc_client_addr.sin_addr));
				entry->u16RemMode = 0;
				entry->u16SyncState = 1;
			} else {
				pthread_mutex_lock(&(entry->lock));
		...
		while(1) {
			...
			if (ret > 0) {
				...
				if (entry == NULL) {
					...
					entry->client_find_count = 0;
					entry->client_no_find_count = 0;
					entry->ipaddr = strdup(_ip);
					entry->u16RemMode = 0;
					entry->u16SyncState = 1;
				}
				if (strncmp(entry->ipaddr, _ip, strlen(_ip))) {
					pthread_mutex_lock(&(entry->lock));
					free(entry->ipaddr);
					entry->ipaddr = strdup(_ip);
					pthread_mutex_unlock(&(entry->lock));
...
void *thread_read_send_baicell_json_object(void *arg)
{
	...
	while (1) {
		...
		if (get_imsi_send_flag()) {
			send_str = cJSON_PrintUnformatted(bc_sta_object);
			if (!send_str) {
				bc_sta_json_obj_unlock();
				continue;
			}
			printf("Timeout,send LTE info.\n");
			write_to_blue_tooth(get_bluetooth_fd(), send_str);
			free(send_str);
		}
			bc_sta_json_delete();
			bc_sta_json_init();
